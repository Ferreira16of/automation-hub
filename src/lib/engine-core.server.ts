// Server-only execution core. Reused by createServerFn and public webhook/scheduler routes.
import { z } from "zod";
...
async function runNode(
  ...
  credentialsById: Record<string, any>
): Promise<any> {
...
export interface ExecuteOpts {
  ...
  triggerData?: any;
  /** Optional pre-loaded workflow row to avoid an extra fetch. */
  workflow?: { id: string; nodes: any; edges: any; user_id?: string };
  /** When true, load credentials by user_id (admin path, RLS bypass). */
  filterCredentialsByUser?: boolean;
}

export async function executeWorkflow(opts: ExecuteOpts) {
  const { supabase, workflowId, userId, trigger, triggerData } = opts;

  let wfRow = opts.workflow;
  if (!wfRow) {
    const { data, error } = await supabase
      .from("workflows")
      .select("id,nodes,edges,user_id")
      .eq("id", workflowId)
      .single();
  ...
  const nodes = (wfRow!.nodes as any[]).map((n) => NodeSchema.parse(n));
  const edges = (wfRow!.edges as any[]).map((e) => EdgeSchema.parse(e));

  let credsQuery = supabase.from("credentials").select("id,data,user_id");
  if (opts.filterCredentialsByUser) credsQuery = credsQuery.eq("user_id", userId);
  const { data: creds } = await credsQuery;
  const credsById: Record<string, any> = {};
  for (const c of creds ?? []) credsById[(c as any).id] = (c as any).data;
  ...
  const logs: LogEntry[] = [];
  const ctx: Record<string, any> = { $trigger: triggerData ?? {}, $nodes: {} };

  const { data: exec } = await supabase
    .from("executions")
    .insert({ workflow_id: wfRow!.id, user_id: userId, status: "running", trigger })
    .select("id")
    .single();

  let current: Node | undefined = start;
  let lastOutput: any = triggerData ?? {};
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (const tok of part.split(",")) {
      if (tok.startsWith("*/")) {
        ...
        if (step > 0 && (value - min) % step === 0) { matched = true; break; }
      } else if (/^\d+$/.test(tok)) {
        if (Number(tok) === value) { matched = true; break; }
      } else if (/^\d+-\d+$/.test(tok)) {
        const [a, b] = tok.split("-").map(Number);
        if (value >= a && value <= b) { matched = true; break; }
      }
      void max;
    }
